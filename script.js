document.addEventListener("DOMContentLoaded", async () => {
  ...
  function setupVotingCategory(categoryName, nextCategoryPage = null) {
    ...
    const confirmationStep = document.getElementById(
      `confirmationStep${categoryName}`,
    );
    const confirmedProjectSpan = document.getElementById(
      `confirmedProject${categoryName}`,
    );
    ...
    voteForm.addEventListener("submit", async (event) => {
      ...
      const projectId = projectIdInput.value.trim();
      const voterId = new URLSearchParams(window.location.search).get(
        "voterId",
      );
      ...
      let allVotos =
        JSON.parse(sessionStorage.getItem("feiramakerVotos")) || [];
      const existingVoteIndex = allVotos.findIndex(
        (v) => v.categoriaId === categoryName,
      );
      ...
      displayVoteMessage(
        voteMessage,
        `Voto na categoria ${categoryName} registrado localmente. Redirecionando...`,
      );
